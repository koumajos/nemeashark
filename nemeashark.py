...
def basic_plugin(rec, biflow):
    ...
    d = [str(rec.SRC_MAC), str(rec.SRC_IP)]
    if src_port != int(rec.SRC_PORT):
        d.append(f"{src_port}({str(rec.SRC_PORT)})")
    else:
        d.append(str(rec.SRC_PORT))
    d.append(str(rec.DST_MAC))
    d.append(str(rec.DST_IP))
    if dst_port != int(rec.DST_PORT):
        d.append(f"{dst_port}({str(rec.DST_PORT)})")
    else:
        d.append(str(rec.DST_PORT))
    d.append(str(packets))
    d.append(str(byte))
    d.append(
    ...
    if protocol != str(rec.PROTOCOL):
        d.append(f"{protocol}({str(rec.PROTOCOL)})")
    else:
        d.append(str(rec.PROTOCOL))
    d.append(tcp_falgs)
    return d
